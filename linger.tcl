...
set TextFont     "-family {courier} -size -14 -weight bold"
...
proc preventRepeat {{key {}}} {
  ...
  bind . <KeyRelease$type> {set _KR(%k) [clock clicks]}
...
proc getRating {item {tag {}}} {
  ...
  set owidth 40
  set y [expr [winfo height .c] / 2 - $UpShift]
  ...
  for {set i 1} {$i <= $RatingChoices} {incr i} {
    ...
    radiobutton .d$i -value $i -variable Difficulty -text $i -command {set StopTime [clock clicks]}
...
proc askQuestions {item font justify} {
  global Question CollectRatings
...
after 10
if {[timeElapsed $time [clock clicks]] > 100} {
  set ClockRes 1000
} else {set ClockRes 1}
